...
BADGE_FILE="$GIT_ROOT/include/badges/python_version.json"
...
PYTHON_VERSION=$(uv run python --version | head -n1 | awk '{print $NF}')

# Update message field in JSON
tmpfile=$(mktemp)
jq --arg ver "$PYTHON_VERSION" '.message=$ver' "$BADGE_FILE" > "$tmpfile" \
  && mv "$tmpfile" "$BADGE_FILE"

# Stage the updated file
git add "$BADGE_FILE"

echo "Updated $BADGE_FILE with Python version: $PYTHON_VERSION"
